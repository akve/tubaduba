...
if ( ! function_exists( 'woocommerce_demo_store' ) ) {
	...
	function woocommerce_demo_store() {
		...
		echo apply_filters( 'woocommerce_demo_store', '<p class="woocommerce-store-notice demo_store">' . wp_kses_post( $notice ) . ' <a href="#" class="woocommerce-store-notice__dismiss-link">' . esc_html__( 'Отменить', 'woocommerce' ) . '</a></p>', $notice );
	}
}
...
if ( ! function_exists( 'woocommerce_template_loop_product_title' ) ) {
	...
	function woocommerce_template_loop_product_title() {
		echo '<a href="'.get_permalink().'" class="woocommerce-loop-product__title">' . get_the_title() . '</a>';
	}
}
...
if ( ! function_exists( 'woocommerce_widget_shopping_cart_proceed_to_checkout' ) ) {
	...
	function woocommerce_widget_shopping_cart_proceed_to_checkout() {
		echo '<a href="' . esc_url( wc_get_checkout_url() ) . '" class="button checkout wc-forward">' . esc_html__( 'Оформить', 'woocommerce' ) . '</a>';
	}
}
		...
		switch ( $args['type'] ) {
			case 'country' :
				...
				if ( 1 === sizeof( $countries ) ) {
					...
					$field = '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="country_to_state country_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . '>' . '<option value="">' . esc_html__( 'Выберите страну&hellip;', 'woocommerce' ) . '</option>';
			...
			case 'state' :
				...
				if ( is_array( $states ) && empty( $states ) ) {
					...
					$field .= '<select name="' . esc_attr( $key ) . '" id="' . esc_attr( $args['id'] ) . '" class="state_select ' . esc_attr( implode( ' ', $args['input_class'] ) ) . '" ' . implode( ' ', $custom_attributes ) . ' data-placeholder="' . esc_attr( $args['placeholder'] ) . '">
						<option value="">' . esc_html__( 'Область&hellip;', 'woocommerce' ) . '</option>';
...
function wc_get_star_rating_html( $rating, $count = 0 ) {
	...
	if ( 0 < $count ) {
		...
		$html .= sprintf( esc_html__( 'Оценка %s из 5', 'woocommerce' ), '<strong class="rating">' . esc_html( $rating ) . '</strong>' );
	}
