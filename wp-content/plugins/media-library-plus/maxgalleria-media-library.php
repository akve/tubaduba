...
class MaxGalleriaMediaLib {
  ...
  public function __construct() {
	...
	public function set_global_constants() {	
		...
		define('MAXGALLERIA_MEDIA_LIBRARY_VERSION_NUM', '3.0.10');
  ...
  public function add_new_attachment($filename, $folder_id, $title_text="", $alt_text="") {
	...
	public function test_scan_attachments () {
    ...
    if(!$uploads_path['error']) {
      ...
      $current_folder = "";
			...
			$bad_url_found = false;
			$output = "";
			$upload_length = strlen($uploads_dir);
      ...
      if($rows) {
        foreach($rows as $row) {
					...
					$attach_id_found = strpos($image_location, 'attachment_id=');
					
					if($attach_id_found == false) {
						
						$new_position = strpos($image_location, $uploads_dir);
						$uploads_location = substr($image_location, 0, $new_position + $upload_length);

						if($base_url != $uploads_location) {
							$output .= "$row->ID: " . $image_location . "<br>";
							$bad_url_found = true;
						}							
		...
		if($bad_url_found) {
			$summary = __('<br><h2>Bad URLs were found in the Media Library:</h2><br>','maxgalleria-media-library') . $output;		  //$summary = "<br><h2>Bad URLs were found in the database:</h2><br>" . $output;
			$result = 'off';
		} else {
			$summary = __('<br>No Bad URLs were found<br>','maxgalleria-media-library');
			update_option('mgmlp_database_checked', 'on', true);
			$result = 'on';
		}	
  ...
  public function setup_mg_media_plus() {
    ...
    add_submenu_page('media-library', __('Image SEO','maxgalleria-media-library'), __('Image SEO','maxgalleria-media-library'), 'upload_files', 'image-seo', array($this, 'image_seo'));
		...
		if(get_option('mgmlp_urls_saved', 'off') === 'on') {
		  add_submenu_page('media-library', __('Restore Original URLs','maxgalleria-media-library'), __('Restore Original URLs','maxgalleria-media-library'), 'upload_files', 'undo-url-fix', array($this, 'undo_url_fix'));
		}
...
<!--          <div class="mgmlp-title" id='mg-prono-top'>
        ...
        <!--<p id='mlp-more-info'><a href='http://maxgalleria.com/media-library-plus/' target="_blank"><?php _e('Click here to learn about the Media Library Plus', 'maxgalleria-media-library' ); ?></a></p>-->
				...
				<div id="scan-results"></div>				
					...
					$load_mlp = get_option('mgmlp_database_checked', 'off');
					if( $load_mlp != 'on')
						$load_mlp = $this->test_scan_attachments();
					
					if($load_mlp == 'off') {
						echo __("<br>In order to run Media Libray Plus these URLs will need to be fixed.<br>", "maxgalleria-media-library") . PHP_EOL;
            echo '<br><a id="mgmlp-fix-media" class="gray-blue-link" >' .  __('Fix bad URLs','maxgalleria-media-library') . '</a>' . PHP_EOL;            
						echo '<div id="alwrap">' . PHP_EOL;
            echo '  <div style="display:none" id="ajaxloader"></div>' . PHP_EOL;
            echo '</div>' . PHP_EOL;
						echo '<div id="url-fix-message"></div>';
					}  else {
...
//            });
//            						
//            jQuery(document).on("click", "li.media-item ", function () {	
//						  console.log('clicked');
//							if(!jQuery(this).hasClass('media-select')) {
...
//    echo '  <div class="media-plus-toolbar wp-filter"><div class="media-toolbar-secondary">' . PHP_EOL;
    ...
    echo '  <h2 class="mgmlp-title">Maxgalleria Media Library Search Results</h2>' . PHP_EOL;
...
<?php
	...
	public function mgmlp_fix_bad_urls() {
    ...
    if(!$uploads_path['error']) {
      ...
      $rows = $wpdb->get_results($sql);
			...
			$table = $wpdb->prefix . "posts";
			$destination_table = $wpdb->prefix . MAXGALLERIA_MEDIA_LIBRARY_BACKUP_TABLE;
      ...
      if($rows) {
        foreach($rows as $row) {
		      ...
		      if($attach_id_found == false) {
						...
						if($base_url != $uploads_location) {
		          update_option('mgmlp_urls_saved', 'on', false);
							
							//save the record
							$sql = "INSERT INTO `$destination_table` SELECT * FROM `$table` where ID = $row->ID;";
              $insert_rows = $wpdb->get_results($sql);

							$file_location = $this->locaton_without_basedir($image_location, $uploads_dir, $upload_length);
							$new_image_location = $base_url . $file_location;

							$data = array('guid' => $new_image_location );
			  ...
			  update_option('mgmlp_database_checked', 'on', true);
		    echo __('The urls have been fixed.','maxgalleria-media-library');
				
        $location = "window.location.href = '" . site_url() . "/wp-admin/admin.php?page=media-library'";
        echo "<script>$location</script>";
  ...
  public function add_post_backup_table () {
    global $wpdb;
    require_once(ABSPATH . 'wp-admin/includes/upgrade.php');

    $table = $wpdb->prefix . MAXGALLERIA_MEDIA_LIBRARY_BACKUP_TABLE;
    $sql = "CREATE TABLE IF NOT EXISTS " . $table . " ( 
  ...
  `comment_count` bigint(20) NOT NULL DEFAULT '0'
      ...
      <div id="wp-media-grid" class="wrap">                
        ...
        <div class="media-plus-toolbar">
					<div class="media-toolbar-secondary">  
						...
						<div id='mgmlp-title-area'>
							<h2 class='mgmlp-title'><?php _e('Media Library Plus - Undo Fixed URLs', 'maxgalleria-media-library' ); ?> </h2>    
							<a id="pro-btn" href="<?php echo UPGRADE_TO_PRO_LINK; ?>" target="_blank">Get MLP Pro</a>
							<!--<a id="pro-btn" href="<?php echo UPGRADE_TO_PRO_LINK; ?>" target="_blank"><img alt="Get MLP Pro" src="<?php echo MAXGALLERIA_MEDIA_LIBRARY_PLUGIN_URL ?>/images/get-mlp-pro.png" width="113" height="27" ></a>-->
							<div id="new-top-promo">
								<a id="mf-top-logo" target="_blank" href="http://maxfoundry.com"><img alt="maxfoundry logo" src="<?php echo MAXGALLERIA_MEDIA_LIBRARY_PLUGIN_URL ?>/images/mf-logo.png" width="140" height="25" ></a>
              ...
              <div style="display:none" id="ajaxloader"></div>
						...
						<p>If you needed to fix the incorrect URLs in the Media Library and now you want to restore those original URLs, click the Restore Original URLs button.</p>
					<div id="ufu-button-row">
						
							<?php 	
								echo '<a id="restore-fixed-urls" class="gray-blue-link" >' .  __('Restore Original URLs','maxgalleria-media-library') . '</a>' . PHP_EOL;            
								//echo '<a id="delete-media-library-backup" class="gray-blue-link" >' .  __('Delete Backedup Media Records','maxgalleria-media-library') . '</a>' . PHP_EOL;            
							?>
	...
	public function mgmlp_restore_backup() {
		
    global $wpdb;
		$post_table = $wpdb->prefix . "posts";
    $backup_table = $wpdb->prefix . MAXGALLERIA_MEDIA_LIBRARY_BACKUP_TABLE;
		
		$sql = "UPDATE $post_table
       JOIN $backup_table
       ON $post_table.ID = $backup_table.ID
       SET $post_table.guid = $backup_table.guid";
	
		$wpdb->get_results($sql);
		
		$wpdb->get_results("TRUNCATE TABLE $backup_table;");
		
		update_option('mgmlp_database_checked', 'off', true);
		update_option('mgmlp_urls_saved', 'off', false);
		
		echo __('The backup was restored.','maxgalleria-media-library');
