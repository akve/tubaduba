...
class Server {
	...
	/**
	 * Infinite scroll REST Api callback.
	 *
	...
	public function get_items( \WP_Rest_Request $request ) {
		...
		if ( empty( $request['page_number'] ) ) {
			return new \WP_REST_Response(
				array(
					'code'    => 'error',
					'message' => esc_html__( 'Product infinite scroll error: Page number is missing.', 'neve' ),
				),
				400
			);
		}
		...
		$markup     = '';
		$query_args = $request->get_json_params();
		if ( ! array_key_exists( 'query', $query_args ) ) {
			return new \WP_REST_Response(
				array(
					'code'    => 'error',
					'message' => esc_html__( 'Product infinite scroll error: Query args are missing.', 'neve' ),
				),
				400
			);
		}
		$args = json_decode( $query_args['query'], true );
		if ( empty( $args ) ) {
			return new \WP_REST_Response(
				array(
					'code'    => 'error',
					'message' => esc_html__( 'Invalid query args.', 'neve' ),
				),
				400
			);
		}
		...
		$this->wish_list_instance->run();
		$this->quick_view_instance->run();
		$this->shop_product_instance->product_image_style();
