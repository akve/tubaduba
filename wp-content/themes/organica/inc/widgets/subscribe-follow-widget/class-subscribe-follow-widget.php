...
class Organica_Subscribe_Follow_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'organica widget-subscribe';
		$this->widget_description = esc_html__( 'Display subscribe form and follow links.', 'organica' );
		$this->widget_id          = 'organica_widget_subscribe_follow';
		$this->widget_name        = esc_html__( 'Subscribe and Follow', 'organica' );
		$this->settings           = array(
			'enable_subscribe' => array(
				...
				'options' => array(
					'enable_subscribe' => esc_html__( 'Enable Subscribe Box', 'organica' ),
				),
			),
			'subscribe_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'organica' ),
				'label' => esc_html__( 'Subscribe Title', 'organica' ),
			),
			'subscribe_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Subscribe text message', 'organica' ),
			),
			'subscribe_input' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Enter your email', 'organica' ),
				'label' => esc_html__( 'Subscribe input placeholder', 'organica' ),
			),
			'subscribe_submit' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Subscribe', 'organica' ),
				'label' => esc_html__( 'Subscribe submit label', 'organica' ),
			),
			'subscribe_success' => array(
				'type'  => 'text',
				'value' => esc_html__( 'You successfully subscribed', 'organica' ),
				'label' => esc_html__( 'Subscribe success', 'organica' ),
			),
			'enable_follow' => array(
				'type'   => 'checkbox',
				'value'  => array(
				...
				'options' => array(
					'enable_follow' => esc_html__( 'Enable Follow Box', 'organica' ),
				),
			),
			'follow_title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Follow', 'organica' ),
				'label' => esc_html__( 'Follow Title', 'organica' ),
			),
			'follow_message' => array(
				'type'  => 'textarea',
				'label' => esc_html__( 'Follow text message', 'organica' ),
			),
		);

		add_action( 'wp_ajax_organica_subscribe', array( $this, 'process_subscribe' ) );
		add_action( 'wp_ajax_nopriv_organica_subscribe', array( $this, 'process_subscribe' ) );

		$this->errors = array(
			'nonce'     => esc_html__( 'Security validation failed', 'organica' ),
			'mail'      => esc_html__( 'Please, provide valid mail', 'organica' ),
			'mailchimp' => esc_html__( 'Please, set up MailChimp API key and List ID', 'organica' ),
			'internal'  => esc_html__( 'Internal error. Please, try again later', 'organica' ),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $subscribe_enabled && $subscribe_template && $api_key && $list_id ) {
			...
			esc_html_e( 'Please set up MailChimp API key and List ID', 'organica' );
		}
	...
	public function get_social_nav() {
		return organica_get_social_list( 'widget' );
	}
	...
	public function get_subscribe_submit( $class ) {
		...
		$subscribe_submit = organica_render_icons( $subscribe_submit );
	...
	public function process_subscribe() {
		...
		if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'organica_subscribe' ) ) {
...
add_action( 'widgets_init', 'organica_register_subscribe_follow_widgets' );
function organica_register_subscribe_follow_widgets() {
	register_widget( 'Organica_Subscribe_Follow_Widget' );
}
