...
if ( ! class_exists( 'Organica_Taxonomy_Tiles_Widget' ) ) {

	class Organica_Taxonomy_Tiles_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Taxonomy Tiles', 'organica' );
			$this->widget_description	= esc_html__( 'This widget displays images from taxonomy.', 'organica' );
			...
			$this->settings = array(
				'title'	=> array(
					...
					'label'				=> esc_html__( 'Widget title', 'organica' ),
				),
				'terms_type' => array(
					'type'				=> 'radio',
					'value'				=> 'category',
				...
				'category'=> array(
					...
					'label'				=> esc_html__( 'Select category to show', 'organica' ),
				...
				'post_tag' => array(
					...
					'label'				=> esc_html__( 'Select tags to show', 'organica' ),
				...
				'description_length' => array(
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Description words length ( Set 0 to hide description. )', 'organica' ),
				),
				'show_post_count' => array(
					...
					'options'		=> array(
						'show_post_count_check' => esc_html__( 'Show post count', 'organica' ),
					),
				),
				'layout_type' => array(
					'type'				=> 'radio',
					...
					'options'			=> array(
						'grid' => array(
							'label'		=> esc_html__( 'Grid', 'organica' ),
						),
						'tiles' => array(
							'label'		=> esc_html__( 'Tiles', 'organica' ),
						),
					),
					'label'				=> esc_html__( 'Choose Layout Type', 'organica' ),
				),
				'columns_number' => array(
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Columns number ( layout type grid )', 'organica' ),
				),
				'items_padding' => array(
					'type'				=> 'stepper',
					'value'				=> '5',
					...
					'min_value'			=> '0',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Items padding ( size in pixels )', 'organica' ),
				),
			);
		...
		public function get_terms_list ( $tax = 'category', $args = array( 'hide_empty' => 0, 'hierarchical' => 0 ) ) {
			...
			if ( $terms ) {
				foreach ( $terms as $term ) {
					$output_terms[ $term->term_id ] = $term->name . sprintf( _n( ' ( 1 post )', ' ( %s posts )', $term->count, 'organica' ), $term->count );
				}
			}
		...
		public function get_term_post_count( $term, $visible ) {
			return ( 'false' === $visible ) ? '' : '<span class="post-count">' . sprintf( _n( '1 post', '%s posts', $term->count, 'organica' ), $term->count ) . '</span>';
		}
		 ...
		 * @return string
		 */
		public function get_term_image( $term, $image_size = 'organica-thumb-m', $image_mobile_size = 'organica-thumb-s', $image_type = 'img' ) {
			$id         = get_term_meta( $term->term_id, 'organica_thumb' , true );
			...
			if ( $id ) {
				...
				$placeholder_attr = apply_filters( 'organica_taxonomy_tiles_widget_placeholder_default_args', array(
					'width'			=> $size_array['width'],
					'height'		=> $size_array['height'],
					'background'	=> $this->hex_to_string( get_theme_mod( 'regular_accent_color_1', organica_theme()->customizer->get_default( 'regular_accent_color_1' ) ) ),
					'foreground'	=> $this->hex_to_string( get_theme_mod( 'regular_accent_color_2', organica_theme()->customizer->get_default( 'regular_accent_color_2' ) ) ),
		...
		public function widget( $args, $instance ) {
			...
			if ( isset( $terms ) && $terms ) {
				$image_size        = apply_filters( 'organica_taxonomy_tiles_widget_size', 'organica-thumb-m', $this->instance );
				$image_mobile_size = apply_filters( 'organica_taxonomy_tiles_widget_size_mobile', 'organica-thumb-s', $this->instance );
				...
				echo apply_filters( 'organica_taxonomy_tiles_widget_before',
				...
				echo apply_filters( 'organica_taxonomy_tiles_widget_after', '</div>', $this->instance );
			}
...
add_action( 'widgets_init', 'organica_register_taxonomy_tiles_widget' );

function organica_register_taxonomy_tiles_widget() {
	register_widget( 'Organica_Taxonomy_Tiles_Widget' );
}
