 ...
 * @package Organica
 */
class Organica_Smart_Slider_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'organica widget_smart_slider';
		$this->widget_description = esc_html__( 'Display smart slider on your site.', 'organica' );
		$this->widget_id          = 'widget_smart_slider';
		$this->widget_name        = esc_html__( 'Smart Slider Widget', 'organica' );
		$this->settings           = array(
			'title' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Smart Slider', 'organica' ),
			...
			'more_button_text' => array(
				'type'  => 'text',
				'value' => esc_html__( 'View All', 'organica' ),
			...
			'width' => array(
				'type'  => 'text',
				'value' => '100%',
				'label' => esc_html__( 'Slider width', 'organica' ),
			),
			'height' => array(
				'type'  => 'text',
				'value' => '827',
				'label' => esc_html__( 'Slider height', 'organica' ),
			),
			'orientation' => array(
				'type'    => 'select',
				'size'    => 1,
				...
				'label' => esc_html__( 'Slider orientation', 'organica' ),
			),
			'slide_distance' => array(
				...
				'max_value' => 100,
				'min_value' => 0,
				'label'     => esc_html__( 'Slide distance(px)', 'organica' ),
			),
			'slide_duration' => array(
				...
				'min_value'  => 100,
				'step_value' => 100,
				'label'      => esc_html__( 'Slide duration(ms)', 'organica' ),
			),
			'slide_fade' => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Use fade effect?', 'organica' ),
			),
			'navigation' => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Use navigation?', 'organica' ),
			),
			'fade_navigation' => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Indicates whether the arrows will fade in only on hover', 'organica' ),
			),
			'pagination' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Use pagination?', 'organica' ),
			),
			'autoplay' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Use autoplay?', 'organica' ),
			),
			'fullScreen' => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display fullScreen button?', 'organica' ),
			),
			'shuffle' => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Indicates if the slides will be shuffled', 'organica' ),
			),
			'loop' => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Use infinite scrolling?', 'organica' ),
			),
			'thumbnails' => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display thumbnails?', 'organica' ),
			),
			'thumbnails_arrows' => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display thumbnails arrows?', 'organica' ),
			),
		);
	...
	public function widget( $args, $instance ) {
		...
		if ( $posts_query ) {
			...
			$html = '<h4>' . esc_html__( 'Posts for slider not found', 'organica' ) . '</h4>';
		}
	...
	public function render_slider( $posts_query ) {
		...
		$slider_html_attr .= ' data-thumbnails-width="247"';
		$slider_html_attr .= ' data-thumbnails-height="107"';

		$html = '<div class="organica-smartslider" ' . $slider_html_attr . '>';
		$html .= '<div id="' . $uniq_id . '" class="organica-smartslider__instance slider-pro">';
		$html .= '<div class="organica-smartslider__slides sp-slides">';
		$html .= $this->get_slider_loop( $posts_query );
		$html .= '</div>';

		if ( 'true' == $this->instance['thumbnails'] ) {
			$html .= '<div class="organica-smartslider__thumbnails sp-thumbnails">';
	...
	public function get_query_slider_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'organica_smart_slider_default_query_args', array(
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => -1,
	...
	public function get_slider_loop( $posts_query ) {
		 ...
		 * @var  array
		 */
		$title_settings = apply_filters( 'organica_smart_slider_title_settings', array(
		 ...
		 * @var  array
		 */
		$content_settings = apply_filters( 'organica_smart_slider_content_settings', array(
		 ...
		 * @var  array
		 */
		$more_settings = apply_filters( 'organica_smart_slider_more_button_settings', array(
		...
		$placeholder_args = apply_filters( 'organica_smart_slider_placeholder_args', array(
			'width'  => 1600,
	...
	public function get_slider_thumbnails( $posts_query ) {
		...
		$thumbnail_size = apply_filters( 'organica_smart_slider_thumbnail_size', 'organica-thumb-237-97', $this->instance );
	...
	public function get_image( $id, $size, $placeholder_attr, $only_url = false ) {
		...
		if ( has_post_thumbnail( $id ) ) {
			...
			$default_placeholder_attr = apply_filters( 'organica_smart_slider_placeholder_default_args', array(
				'width'      => 900,
				'height'     => 500,
				'background' => 'eef4fa',
				'foreground' => '298ffc',
...
add_action( 'widgets_init', 'organica_register_smart_slider_widgets' );
function organica_register_smart_slider_widgets() {
	register_widget( 'Organica_Smart_Slider_Widget' );
}