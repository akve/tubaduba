 ...
 * @package Organica
 */
class Organica_Carousel_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		$this->widget_cssclass    = 'organica widget_carousel';
		$this->widget_description = esc_html__( 'Display a list of your posts on your site.', 'organica' );
		$this->widget_id          = 'widget_carousel';
		$this->widget_name        = esc_html__( 'Carousel Widget', 'organica' );
		$this->settings           = array(
			'title'                => array(
				'type'  => 'text',
				'value' => esc_html__( 'Carousel', 'organica' ),
			...
			'terms_type'           => array(
				...
				'options' => array(
					'category' => array(
						'label' => esc_html__( 'Category', 'organica' ),
						'slave' => 'terms_type_category',
					),
					'post_tag' => array(
						'label' => esc_html__( 'Tag', 'organica' ),
						'slave' => 'terms_type_post_tag',
					),
				),
				'label'   => esc_html__( 'Choose taxonomy type', 'organica' ),
			),
			'categories'           => array(
				'type'             => 'select',
				'size'             => 1,
				...
				'options'          => false,
				'label'            => esc_html__( 'Select category', 'organica' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select category', 'organica' ),
			...
			'tags'                 => array(
				...
				'options'          => false,
				'label'            => esc_html__( 'Select tags', 'organica' ),
				'multiple'         => true,
				'placeholder'      => esc_html__( 'Select tags', 'organica' ),
			...
			'posts_per_page'       => array(
				...
				'max_value' => 20,
				'min_value' => 1,
				'label'     => esc_html__( 'Posts count', 'organica' ),
			),
			'post_title'           => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display title', 'organica' ),
			),
			'content'              => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display content', 'organica' ),
			),
			'post_cats'            => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display categories', 'organica' ),
			),
			'post_author'          => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display author', 'organica' ),
			),
			'post_comments'        => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display comments', 'organica' ),
			),
			'more_button'          => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Display more button', 'organica' ),
			),
			'more_button_text'     => array(
				'type'  => 'text',
				'value' => esc_html__( 'Read more', 'organica' ),
				'label' => esc_html__( 'More button text', 'organica' ),
			),
			'trim_words'           => array(
				'type'       => 'slider',
				'value'      => 25,
				...
				'min_value'  => 1,
				'step_value' => 1,
				'label'      => esc_html__( 'Content words trimmed count', 'organica' ),
			),
			'slides_per_view'      => array(
				...
				'min_value' => 1,
				'value'     => 2,
				'label'     => esc_html__( 'Number of slides per view', 'organica' ),
			),
			'slides_per_group'     => array(
				...
				'min_value' => 1,
				'value'     => 1,
				'label'     => esc_html__( 'Number slides per group', 'organica' ),
			),
			'slides_per_column'    => array(
				...
				'min_value' => 1,
				'value'     => 1,
				'label'     => esc_html__( 'Multi Row Slides Layout', 'organica' ),
			),
			'space_between_slides' => array(
				...
				'min_value' => 0,
				'value'     => 30,
				'label'     => esc_html__( 'Width of the space between slides(px)', 'organica' ),
			),
			'duration_speed'       => array(
				...
				'min_value' => 100,
				'value'     => 500,
				'label'     => esc_html__( 'Duration of transition between slides (ms)', 'organica' ),
			),
			'navigation'           => array(
				'type'  => 'switcher',
				'value' => 'true',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Slider navigation', 'organica' ),
			),
			'pagination'           => array(
				'type'  => 'switcher',
				'value' => 'false',
				'style' => ( wp_is_mobile() ) ? 'normal' : 'small',
				'label' => esc_html__( 'Slider pagination', 'organica' ),
			),
		);
	...
	public function widget( $args, $instance ) {
		...
		$footer_widget_columns = get_theme_mod( 'footer_widget_columns', organica_theme()->customizer->get_default( 'footer_widget_columns' ) );
		...
		$data_attr_line = 'class="organica-carousel swiper-container"';
		...
		if ( $posts_query ) {
			...
			echo '<h5>' . esc_html__( 'Posts not found', 'organica' ) . '</h5>';
		}
	...
	public function get_query_items( $query_args = array() ) {
		...
		$defaults_query_args = apply_filters( 'organica_carousel_default_query_args', array(
			'post_type'      => 'post',
			'orderby'        => 'date',
			'order'          => 'DESC',
			'posts_per_page' => - 1,
	...
	public function get_carousel_loop( $posts_query ) {
		$image_size = apply_filters( 'organica_carousel_image_size', 'organica-thumb-570-370', $this->instance, $this->args );

		$view_dir = locate_template( 'inc/widgets/tm-carousel-widget/views/tm-carousel-view.php' );

		if ( ! $view_dir ) {
			return '<h5>' . esc_html__( 'View file not found', 'organica' ) . '</h5>';
		}
		...
		while ( $posts_query->have_posts() ) : $posts_query->the_post();
			...
			$placeholder_args = apply_filters( 'organica_carousel_placeholder_args', array(
				'width'      => 560,
				'height'     => 350,
				'background' => '000',
				'foreground' => 'fff',
			...
			$title            = ( 'true' == $this->instance['post_title'] ) ? '<h2><a href="' . esc_url( $permalink ) . '">' . $title . '</a></h2>' : '';
			...
			$date             = '<span class="post__date">' . esc_html__( 'Published on ', 'organica' ) . '<a href="' . esc_url( $permalink ) . '">' . $this->get_post_date( $post_id ) . '</a></span>';
			$author           = ( 'true' == $this->instance['post_author'] ) ? '<span class="post__author vcard">' . esc_html__( 'by ', 'organica' ) . $this->get_post_author( $post_id ) . '</span>' : '';
			$comments         = ( 'true' == $this->instance['post_comments'] ) ? '<span class="post__comments">' . $this->get_post_comments( $post_id ) . '</span>' : '';
			$terms_line       = ( 'true' == $this->instance['post_cats'] ) ? '<div class="post__cats">' . $this->get_terms_line( $post_id, $this->instance['terms_type'] ) . '</div>' : '';
	...
	public function get_image( $id, $size, $placeholder_attr, $only_url = false ) {
		...
		if ( has_post_thumbnail( $id ) ) {
			...
			$default_placeholder_attr = apply_filters( 'organica_carousel_placeholder_default_args', array(
				'width'      => 900,
				'height'     => 500,
				'background' => '000',
				'foreground' => 'fff',
	...
	public function get_post_comments( $post_id ) {
		...
		$title_comments = $comments . _n( 'Comment', 'Comments', $comments, 'organica' );
		$comments       = sprintf( '<span class="post-comments-link"><a href="%1$s">%2$s</a></span>', esc_url( get_comments_link() ), $title_comments );
	...
	public function get_post_date( $post_id ) {
		...
		return sprintf( '<time class="post-date" datetime="%1$s">%2$s</time>', esc_attr( get_the_date( 'c' ) ), esc_html( get_the_date() ) );
	}
...
add_action( 'widgets_init', 'organica_register_carosel_widgets' );

function organica_register_carosel_widgets() {
	register_widget( 'Organica_Carousel_Widget' );
}