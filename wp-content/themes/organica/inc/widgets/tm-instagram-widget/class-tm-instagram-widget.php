 ...
 * @package Organica
 */
if ( ! class_exists( 'Organica_Instagram_Widget' ) ) {
	class Organica_Instagram_Widget extends Cherry_Abstract_Widget {
		/**
		...
		public function __construct() {
			...
			$this->widget_description = esc_html__( 'Display a list of photos from Instagram network.', 'organica' );
			$this->widget_id          = 'organica_widget_instagram';
			$this->widget_name        = esc_html__( 'Instagram', 'organica' );
			$this->settings           = array(
				'title'  => array(
					'type'  => 'text',
					'value' => esc_html__( 'Instagram', 'organica' ),
					'label' => esc_html__( 'Title', 'organica' ),
				),
				'tag' => array(
					'type'  => 'text',
					'value' => '',
					'label' => esc_html__( 'Hashtag (enter without `#` symbol)', 'organica' ),
				),
				'image_counter' => array(
					'type'       => 'stepper',
					'value'      => '6',
					...
					'min_value'  => '1',
					'step_value' => '1',
					'label'      => esc_html__( 'Number of photos', 'organica' ),
				),
				'display_caption' => array(
					...
					'options' => array(
						'display_caption_check' => esc_html__( 'Caption', 'organica' ),
					),
				),
				'display_date' => array(
					'type'  => 'checkbox',
					...
					'options' => array(
						'display_date_check' => esc_html__( 'Date', 'organica' ),
					),
				),
			);
		...
		public function widget( $args, $instance ) {
			...
			if ( empty( $instance['tag'] ) ) {
				return print $args['before_widget'] . esc_html__( 'Please, enter #hashtag.', 'organica' ) . $args['after_widget'];
			}
			ob_start();
			...
			if ( ! $photos ) {
				return print $args['before_widget'] . esc_html__( 'No photos. Maybe you entered a invalid hashtag.', 'organica' ) . $args['after_widget'];
			}
			$template = locate_template( 'inc/widgets/tm-instagram-widget/views/instagram.php', false, false );
			printf( '<div class="%s">',
				join( ' ', apply_filters( 'organica_instagram_widget_wrapper_class', array( 'instagram__items' ) ) )
			);
		...
		public function get_photos( $data, $img_counter, $config ) {
			 ...
			 * @var string
			 */
			$order = apply_filters( 'organica_instagram_widget_order', 'media' );
			...
			foreach ( $nodes as $photo ) {
				...
				if ( in_array( 'caption', $config ) ) {
					$_photo['caption'] = wp_html_excerpt(
						$photo['caption'],
						apply_filters( 'organica_instagram_widget_caption_length', 10 ),
						apply_filters( 'organica_instagram_widget_caption_more', '&hellip;' )
					);
				}
		...
		public function get_tags_url() {
			return apply_filters( 'organica_instagram_widget_get_tags_url', $this->service_url . 'explore/tags/%s/' );
		}
		/**
		...
		public function get_post_url() {
			return apply_filters( 'organica_instagram_widget_get_post_url', $this->service_url . 'p/%s/' );
		}
		/**
		 * Clear cache.
		 *
	...
	add_action( 'widgets_init', 'organica_register_instagram_widget' );
	function organica_register_instagram_widget() {
		register_widget( 'Organica_Instagram_Widget' );
	}
}