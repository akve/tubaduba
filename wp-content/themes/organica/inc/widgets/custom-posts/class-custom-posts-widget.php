<?php
class Organica_Custom_Posts_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		$this->widget_name = esc_html__( 'Custom Posts', 'organica' );
		$this->widget_description = esc_html__( 'Display custom posts your site.', 'organica' );

		$this->settings = array(
			'title' => array(
				'type'				=> 'text',
				'value'				=> esc_html__( 'Custom Posts', 'organica' ),
			...
			'terms_type' => array(
				...
				'options'			=> array(
					...
					'post_format'		=> array(
						'label'		=> esc_html__( 'Post Format', 'organica' ),
			...
			'post_format' => array(
				...
				'options'			=> false,
				'label'				=> esc_html__( 'Select post format', 'organica' ),
				'multiple'			=> true,
				'placeholder'		=> esc_html__( 'Select post format', 'organica' ),
			...
			'posts_per_page' => array(
				...
				'max_value'			=> 50,
				'min_value'			=> 0,
				'label'				=> esc_html__( 'Posts count ( Set 0 to show all. )', 'organica' ),
			),
			'post_offset' => array(
				'type'				=> 'stepper',
				'value'				=> '0',
				...
				'min_value'			=> '0',
				'step_value'		=> '1',
				'label'				=> esc_html__( 'Offset post', 'organica' ),
			),
			'title_length' => array(
				'type'				=> 'stepper',
				'value'				=> '10',
				...
				'min_value'			=> '0',
				'step_value'		=> '1',
				'label'				=> esc_html__( 'Title words length ( Set 0 to hide title. )', 'organica' ),
			),
			'excerpt_length' => array(
				...
				'min_value'			=> '0',
				'step_value'		=> '1',
				'label'				=> esc_html__( 'Excerpt words length ( Set 0 to hide excerpt. )', 'organica' ),
			),
			'mate_data' => array(
				'type'				=> 'checkbox',
				'value'				=> array(
				...
				'options'				=> array(
					'date'				=> esc_html__( 'Date', 'organica' ),
					'author'			=> esc_html__( 'Author', 'organica' ),
					'comment_count'		=> esc_html__( 'Comment count', 'organica' ),
					'category'			=> esc_html__( 'Category', 'organica' ),
					'post_tag'			=> esc_html__( 'Tag', 'organica' ),
					'more_button'		=> esc_html__( 'More Button', 'organica' ),
				),
				'label'				=> esc_html__( 'Display post meta data', 'organica' ),
			),
			'button_text' => array(
				'type'				=> 'text',
				'value'				=> 'Read More',
				'label'				=> esc_html__( 'Post read more button label', 'organica' ),
			),
		);
	...
	public function cut_text( $text, $length ) {
		return ( '0' !== $length ) ? wp_trim_words( $text, $length, esc_html__( ' ...', 'organica' ) ) : '' ;
	}
	...
	public function get_post_date( $visible = 'true' ) {
		if ( $visible === 'true') {
			...
			$date_icon = apply_filters( 'organica_custom_posts_date_icon', '<i class="material-icons">schedule</i>' );
			$permalink = $this->get_post_permalink();
	...
	public function get_the_author( $user_id, $visible = 'true' ) {
		if ( $visible === 'true' ) {
			...
			$html= '<div class="post-author"><span>' . esc_html__( 'Posted by ', 'organica' ) .'</span>' . $author . '</div>';
	 ...
	 * @return string
	 */
	public function get_post_image( $post, $image_size = 'organica-thumb-m', $image_mobile_size = 'organica-thumb-s' ) {
		...
		if( $post_thumbnail_id ){
			...
			$placeholder_attr = apply_filters( 'organica_custom_posts_placeholder_default_args',
				array(
					'width'			=> $size['width'],
					'height'		=> $size['height'],
					'background'	=> get_theme_mod( 'regular_accent_color_1', organica_theme()->customizer->get_default( 'regular_accent_color_1' ) ),
					'foreground'	=> get_theme_mod( 'regular_accent_color_2', organica_theme()->customizer->get_default( 'regular_accent_color_2' ) ),
	...
	public function get_button( $text = '', $visible = 'true' ){
		if ( $visible === 'true' ) {
			$permalink = $this->get_post_permalink();
			$icon = apply_filters( 'organica_custom_posts_read_more_icon', '' ); //<span class="btn__icon"></span>
	...
	public function widget( $args, $instance ) {
		...
		if ( $posts ) {
			...
			$image_size = apply_filters( 'organica_playlist_thumbnail_image_size', 'organica-thumb-m' );
			$image_mobile_size = apply_filters( 'organica_playlist_thumbnail_size_mobile', 'organica-thumb-s' );
...
add_action( 'widgets_init', 'organica_register_custom_posts_widget' );

function organica_register_custom_posts_widget() {
	register_widget( 'Organica_Custom_Posts_Widget' );
}