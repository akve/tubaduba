...
if ( ! class_exists( 'Organica_Image_Grid_Widget' ) ) {

	class Organica_Image_Grid_Widget extends Cherry_Abstract_Widget {
		...
		public function __construct() {
			$this->widget_name			= esc_html__( 'Image Grid', 'organica' );
			$this->widget_description	= esc_html__( 'This widget displays images from post.', 'organica' );
			...
			$this->settings				= array(
				...
				'category_name' => array(
					...
					'label'				=> esc_html__( 'Select categories to show', 'organica' ),
				...
				'post_sort' => array(
					...
					'value'				=> 'date',
					'options'			=> array(
						'date' 				=> esc_html__( 'Publish Date', 'organica' ),
						'title'				=> esc_html__( 'Post Title', 'organica' ),
						'comment_count'		=> esc_html__( 'Comment Count', 'organica' ),
					),
					'label'				=> esc_html__( 'Post sorted', 'organica' ),
				),
				'post_number' => array(
					'type'				=> 'stepper',
					'value'				=> '5',
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Posts number', 'organica' ),
				),
				'post_offset' => array(
					'type'				=> 'stepper',
					'value'				=> '0',
				...
				'columns_number' => array(
					...
					'min_value'			=> '1',
					'step_value'		=> '1',
					'label'				=> esc_html__( 'Columns number', 'organica' ),
				),
				'items_padding' => array(
					'type'				=> 'stepper',
					'value'				=> '5',
		...
		public function get_post_image( $post, $image_size ) {
			...
			if ( ! $image ) {
				...
				$placeholder_attr = apply_filters( 'organica_image_grid_widget_placeholder_default_args', array(
					'width'			=> $size['width'],
					'height'		=> $size['height'],
					'background'	=> '000',
					'foreground'	=> 'fff',
		...
		public function widget( $args, $instance ) {
			...
			$args = apply_filters( 'organica_image_grid_widget_args', $args, $instance );
			...
			if ( array_key_exists( $terms_type, $instance ) ) {
				...
				if ( $post_taxonomy ) {
					$post_args = array(
						...
						'order'			=> apply_filters( 'organica_order_image_grid_widget', 'DESC', $instance ), //ASC
			...
			if ( isset( $posts ) && $posts ) {
				...
				echo apply_filters( 'organica_image_grid_widget_before', '<div class="row columns-number-' . $columns_number . '" ' . $row_inline_style . '>', $instance );

				$image_size = apply_filters( 'organica_image_grid_widget_size', 'organica-thumb-570-370', $instance );
				...
				echo apply_filters( 'organica_image_grid_widget_after', '</div>', $instance );
			}
	...
	add_action( 'widgets_init', 'organica_register_image_grid_widget' );
	function organica_register_image_grid_widget() {
		register_widget( 'Organica_Image_Grid_Widget' );
	}
}
