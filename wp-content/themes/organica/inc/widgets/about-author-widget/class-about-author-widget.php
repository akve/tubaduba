...
class Organica_About_Author_Widget extends Cherry_Abstract_Widget {
	...
	public function __construct() {
		...
		$this->widget_cssclass    = 'organica widget-about-author';
		$this->widget_description = esc_html__( 'Display an information about selected user.', 'organica' );
		$this->widget_id          = 'organica_widget_about_author';
		$this->widget_name        = esc_html__( 'About Author', 'organica' );
		$this->settings           = array(
			'title'  => array(
				'type'  => 'text',
				'value' => esc_html__( 'About Author', 'organica' ),
			...
			'user_id' => array(
				...
				'options'          => false,
				'label'            => esc_html__( 'Select user to show', 'organica' ),
			),
			'avatar_size' => array(
				'type'       => 'slider',
				'max_value'  => 512,
				...
				'value'      => 250,
				'step_value' => 1,
				'label'      => esc_html__( 'Author avatar size (set 0 to hide avatar, applied only for Gravatar)', 'organica' ),
			),
			'avatar_img' => array(
				'type'               => 'media',
				'value'              => '',
				'multi_upload'       => false,
				'library_type'       => 'image',
				'upload_button_text' => esc_html__( 'Select image', 'organica' ),
				'label'              => esc_html__( 'Custom avatar image (override default user avatar)', 'organica' ),
			),
			'link' => array(
				'type'  => 'text',
				'value' => '',
				'label' => esc_html__( 'Link (leave empty to hide)', 'organica' ),
			),
			'link_text' => array(
				'type'  => 'text',
				'value' => esc_html__( 'Read More', 'organica' ),
				'label' => esc_html__( 'Link label', 'organica' ),
			),
		);
	...
	public function get_users_list() {
		...
		$result = array( '0' => esc_html__( 'Select a user', 'organica' ) );
...
add_action( 'widgets_init', 'organica_register_about_author_widgets' );
function organica_register_about_author_widgets() {
	register_widget( 'Organica_About_Author_Widget' );
}
