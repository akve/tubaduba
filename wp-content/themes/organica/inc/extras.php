 ...
 * @return string
 */
function organica_set_sidebar_position( $position ) {
	$queried_obj = apply_filters( 'organica_queried_object_id', false );
...
add_filter( 'theme_mod_sidebar_position', 'organica_set_sidebar_position' );
 ...
 * @return string
 */
function organica_render_macros( $string ) {
	$macros = apply_filters( 'organica_data_macros', array(
		'/%%year%%/'           => date( 'Y' ),
		'/%%date%%/'           => date( get_option( 'date_format' ) ),
		'/%%site-name%%/'      => '<span class="sitename">' . get_option( 'blogname' ) . '</span>',
		'/%%privacy-policy%%/' => organica_get_privacy_link(),
	) );
 ...
 * @return string
 */
function organica_get_privacy_link() {
 ...
 * @return string
 */
function organica_render_icons( $content ) {

	$icons     = organica_get_render_icons_set();
	$icons_set = implode( '|', array_keys( $icons ) );

	$regex = '/icon:(' . $icons_set . ')?:?([a-zA-Z0-9-_]+)/';

	return preg_replace_callback( $regex, 'organica_render_icons_callback', $content );
}
 ...
 * @return string
 */
function organica_render_icons_callback( $matches ) {
 ...
 * @return array
 */
function organica_get_render_icons_set() {
	return apply_filters( 'organica_render_icons_set', array(
 ...
 * @return string
 */
function organica_render_theme_url( $url ) {
 ...
 */
function organica_get_image_id_by_url( $image_src ) {
	global $wpdb;
...
function organica_post_formats_gallery() {
	if ( ! in_array( get_theme_mod( 'blog_layout_type' ), array( 'masonry-2-cols', 'masonry-3-cols' ) ) ) {
		return do_action( 'cherry_post_format_gallery' );
	}

	$images = organica_theme()->get_core()->modules['cherry-post-formats-api']->get_gallery_images( false );
